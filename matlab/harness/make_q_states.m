function Q_lib = make_q_states(N_q_list)
% MAKE_Q_STATES  Build a library of named Q state matrices for test cases.
%   Q_lib = make_q_states(N_q_list) returns a struct of Q matrices for each
%   N_q in N_q_list (default [4, 8, 16]).
...
for N_q = N_q_list
    ...
    % 2. Post-90-deg RF about x: all magnetization in F+(0)
    Q = zeros(3, N_q);
    Q(3, 1) = 1.0;
    Q = sim_rf(Q, pi/2, 0);
    ...
    Q_rand = (randn(3, N_q) + 1i * randn(3, N_q)) * 0.1;
